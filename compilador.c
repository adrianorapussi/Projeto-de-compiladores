...
/**
 * Executa o fluxo de an√°lise l√©xica
 */
...
/**
 * -------------------------------------------------------------------------------------------------------------------
 * Fun√ßao que remove as linhas de comentario do codigo
 * -------------------------------------------------------------------------------------------------------------------
 */
void removerLinhaComentario() {
...
/**
 * -------------------------------------------------------------------------------------------------------------------
 * Fun√ß√£o que grava a string recebida em um arquivo output de nome representado pela variavel TOKEN_FILE
 * -------------------------------------------------------------------------------------------------------------------
 */
void escreverArquivo(char *tk) {
    FILE *fp;
...
/**
 * -------------------------------------------------------------------------------------------------------------------
 * Fun√ß√£o que realiza a leitura do arquivo leitura.txt e armazena o conteudo do arquivo em um buffer
 * -------------------------------------------------------------------------------------------------------------------
 */
void lerArquivo(char *nomeDoArquivo) {
    ...
    while (true) {
        ...
        if (feof(fp)) {
            ...
            //quando encontrar \n com ignorarlinha ligado concatena espa√ßo
...
void geradorTokens(char *tk) {
    ...
    // NUMEROS -> num
    char number[1024];
    //Limpa area de memoria para concatena√ßao de numeros
    ...
    for (i = 0; tk[i] != '\0'; i++) {
        ...
        // Caso nao seja aceito por nenhuma das condi√ßoes devo rejeitar e encerrar a execu√ßao
        else {
            printf("ERRO LEXICO: %s\n", tk);
            exit(-1);
        }
...
/**
* FunÁ„o para comparar o valor do TOKEN com o valor esperado retornando TRUE caso sejam iguais ou false caso contrario
*/
bool match(char *tk, char *word) {
    if (strcmp(tk, word) == 0) {
        return true;
...
/**
 * Valida se a sequencia de tokens representa parametros de funÁ„o ou procedimento
*/
bool PARAMS() {
    nonTerminalStart();
...
/**
 * Valida se a sequencia de tokens representa a chamada de uma funÁ„o publica de uma classe
*/
bool PEXP2() {
    nonTerminalStart();
...
/**
 * Valida se a sequencia de tokens representa a chamada de uma funÁ„o interna
*/
bool PEXP() {
    nonTerminalStart();
    if (pegarProximo("ABRE_PARENTESES")) {
